...
const UserCabinetCategoryForm: React.FC<Props> = ({
  ...
  const onCropHandler = useCallback(async () => {
    ...
    try {
      const croppedImage = await getCroppedImg(mainImagePreview, croppedAreaPixels);
      setMainImage(new File([croppedImage], mainImage!.name, {
        type: mainImage!.type,
        lastModified: Date.now(),
      }));
  ...
  return (
      <form onSubmit={handleSubmit(onSubmit)} className="w-full flex flex-wrap gap-[20px]">
        <div className="w-full md:w-[calc(50%-10px)] flex flex-col gap-2">
          ...
          >
            ...
            {isMainDragActive ? (
                <p>{t("admin.adminInfo.adminInfoCategories.adminInfoCategoriesPlaceholder1")}</p>
            ) : (
                <p>{t("admin.adminInfo.adminInfoCategories.adminInfoCategoriesPlaceholder1")}</p>
            )}
          </AdminImage>
          {mainImagePreview && (
              <>
                <div className="w-12 h-12 mr-4">
                  <img src={mainImagePreview} alt="banner preview" className="w-full h-full object-cover" />
                </div>
                {openCrop && (
        ...
        <div className="w-full md:w-[calc(50%-10px)] flex flex-col gap-2">
          ...
          <input
              ...
              placeholder={t("admin.adminInfo.adminInfoCategories.adminInfoCategoriesLabel2")}
        ...
        <div className="w-full flex md:flex-row flex-col gap-[20px] pt-4 border-t border-gray-300">
          ...
          >
            {isLoading
                ? t("admin.adminInfo.adminInfoCategories.adminInfoCategoriesButtonLoading")
                : t("admin.adminInfo.adminInfoCategories.adminInfoCategoriesButtonConfirm")}
          </button>
          <button
              ...
              disabled={isLoading}
          >
            {t("admin.adminInfo.adminInfoCategories.adminInfoCategoriesButtonCancel")}
          </button>
        </div>
      </form>
  );
