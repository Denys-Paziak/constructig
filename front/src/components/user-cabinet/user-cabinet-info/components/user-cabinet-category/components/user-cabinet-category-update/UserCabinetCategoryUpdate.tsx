...
const UserCabinetCategoryUpdate: React.FC = () => {
  ...
  const [categoryImagesPreview, setCategoryImagesPreview] = useState<string[] | null>(null);
  ...
  const onCropHandler = async () => {
    const croppedImage = await getCroppedImg(categoryImagesPreview![0], croppedAreaPixels);
    const uniqueFileName = `cropped_${Date.now()}_${Math.random().toString(36).substring(2, 10)}.jpg`;
    const croppedFile = new File([croppedImage], uniqueFileName, { type: categoryImages[0].type });
