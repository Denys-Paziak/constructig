...
const ServicesEdit: React.FC<Props> = ({ data, sectionName, handleInputChange }) => {
  const { id } = useParams();
    ...
    async (index: number, acceptedFiles: File[]) => {
      ...
      if (token) {
        try {
          ...
          updatedServices[index] = { ...updatedServices[index], image: res.url };
      ...
      {sectionName === "services" && (
        <div className="w-full bg-white rounded-md shadow-md p-3 flex items-start gap-4 flex-col">
          {data.services.cols.map((service: Service, index: number) => {
            // Деструктуруємо необхідні змінні з useDropzone
                  ...
                  <div {...getRootProps()} className="w-full">
                    <AdminImage>
                      ...
                      {service.image ? (
                        ...
                        <p>{isDragActive ? "Перетягніть сюди файли..." : "Перетягніть сюди файли"}</p>
                      )}
                    </AdminImage>
                  </div>
                  {service.image && (
                    <span
                      ...
                      className="w-6 h-6 rounded-full bg-blue-300 p-1.5 cursor-pointer"
                    >
                      <img
                        className="w-full"
